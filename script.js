...
/* =========================================================
   ...
   body{overflow:hidden} stops Chrome's native pull-to-refresh (it only
   triggers when the *document* scrolls). This reproduces the gesture on
   the internal .content scroller instead.
   ========================================================= */
(function initPullToRefresh(){
  const THRESHOLD = 72; // px of pull needed to trigger a refresh
  let startY = 0, pulling = false, dist = 0;
  let scroller = null, indicator = null, label = null;

  function getScroller(){
    // Whichever panel is actually visible/scrollable right now.
  ...
  document.addEventListener('touchstart', (e)=>{
    ...
    if(!scroller) return;
    // Only start tracking if the scroller (or the page) is already at the top.
    if(scroller.scrollTop > 0) return;
    if(e.touches.length !== 1) return;
    startY = e.touches[0].clientY;
    pulling = true; dist = 0;
    indicator = document.getElementById('ptr-indicator');
    label = document.getElementById('ptr-label');
  }, {passive:true});
  ...
  document.addEventListener('touchmove', (e)=>{
    ...
    if(dist <= 0){ if(indicator){ indicator.style.height='0px'; indicator.classList.remove('visible'); } return; }
    if(!indicator) return;
    const capped = Math.min(dist, THRESHOLD * 1.6);
    indicator.classList.add('visible');
    indicator.style.height = Math.min(capped, THRESHOLD) + 'px';
    label.textContent = dist > THRESHOLD ? 'Buraxıb yeniləyin' : 'Yeniləmək üçün çəkin';
  }, {passive:true});
